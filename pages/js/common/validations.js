...
function validate_userName(auth_reg_userName){
  var surName = $("#"+auth_reg_userName).val();
  var status = false;
  if(surName.length>0){ 
    status = true;VALIDATION_MESSAGE_ERROR+='';
    bootstrap_formField_trigger('success',auth_reg_userName); 
  } else {  
    bootstrap_formField_trigger('error',auth_reg_userName);
	VALIDATION_MESSAGE_ERROR+=' Username,';
  }
  return status;
}
